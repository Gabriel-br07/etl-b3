#!/usr/bin/env python3
"""CLI entry-point for the B3 Negócios Consolidados Playwright scraper.
...
Run for a specific date:
    ...
    python scripts/run_b3_scraper_negocios.py --date 2024-06-14

Explicit headed (visual) debug run with slow interactions:

    python scripts/run_b3_scraper_negocios.py --no-headless --slow-mo 500 --screenshots

Full debug with trace recording:

    python scripts/run_b3_scraper_negocios.py --no-headless --slow-mo 800 --screenshots --traces

Playwright Inspector (step through actions manually):

    $env:PWDEBUG="1"; python scripts/run_b3_scraper_negocios.py --no-headless
    # Linux/macOS: PWDEBUG=1 python scripts/run_b3_scraper_negocios.py --no-headless
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Download B3 Negócios Consolidados do Pregão CSV files using a Playwright browser.",
...
def main() -> None:
    ...
    logger.info(
        "B3 Negócios Consolidados scraper starting — date=%s  headless=%s  slow_mo=%s  "
    ...
    logger.info("B3 Negócios Consolidados scraper finished — %d file(s) downloaded.", len(results))
